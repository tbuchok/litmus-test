...
LitmusTest.prototype._flush = function(done) {
  ...
  self.on('htmlProcessed', function(html) {
    self.processedHTML = html;
    var options = { 
  ...
  self.on('imagesProcessed', function(html) {
    var url = path.join('file:', process.cwd(), self.htmlFile);
    juice.juiceContent(html, { url: url }, function(err, processed) {
      if (err)
        return self.emit('error', err);

      self.emit('htmlProcessed', processed)
    });
  });
  ...
  self.once('imagesUploaded', function() {
    ...
    jsdom.env(data, function(err, window) {
      ...
      self.emit('imagesProcessed', document.documentElement.outerHTML);
    });
  });
